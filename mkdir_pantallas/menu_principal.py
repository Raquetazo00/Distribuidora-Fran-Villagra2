...
class MenuPrincipalScreen(BoxLayout):
    ...
    def previsualizar_venta(self):
        ...
        btn_confirmar = Button(
            text="Ir a Facturación", background_color=(0.1, 0.6, 0.3, 1),
    ...
    def confirmar_venta(self, popup):
        """Navega a la pantalla de facturación con los productos seleccionados"""
        popup.dismiss()
        ...
        # Navegar a facturación con los productos actuales
        from mkdir_pantallas.facturacion import FacturacionScreen
        pantalla_facturacion = FacturacionScreen(
            productos_iniciales=self.carrito[:],  # Copiar lista de productos
            cantidades_iniciales=self.cantidades.copy()  # Copiar dict de cantidades
        )
        self.clear_widgets()
        self.add_widget(pantalla_facturacion)
